...
if (!(Get-ItemProperty -erroraction ignore -Path "HKCU:\Software\Microsoft\Internet Explorer\Main" -Name "DisableFirstRunCustomize")) {
 Set-ItemProperty -erroraction ignore -Path "HKCU:\Software\Microsoft\Internet Explorer\Main" -Name "DisableFirstRunCustomize" -Value 2}
if (!($_HideBanner)) {(((iwr -usebasicparsing -erroraction ignore "$github/customizations/banner.html").parsedhtml).body).innertext}
